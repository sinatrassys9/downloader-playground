...
class UpdateChecker(QObject):
    ...
    def __init__(self):
        ...
        self.github_api_url = "https://api.github.com/repos/streamripper/TikTokDownloadExtension/releases/latest"
        self.headers = {
            "User-Agent": f"TikTokLabsDownloadExtension/{VERSION} ({platform.system()} {platform.release()})",
    ...
    def check_for_updates(self):
        ...
        try:
            response = requests.get(self.github_api_url, headers=self.headers, timeout=25)
                ...
                if status_type == "up-to-date":
                    ...
                    version_label.setStyleSheet("""
                        QLabel {
                            ...
                            padding: 6px 16px;
                            border-radius: 15px;
                            background: rgba(255, 194, 7, 0.1);
                            font-size: 12pt;
                            font-weight: bold;
                            border: 1px solid rgba(255, 194, 7, 0.2);
                            margin: 5px;
                        }
                        QLabel:hover {
                            background: rgba(255, 194, 7, 0.15);
                            border: 1px solid rgba(255, 194, 7, 0.4);
                        }
                    """)
